...
func (pm *PinningManager) TrustCertificate(certHash, serverIP, displayName, authCode string) {
	...
	if config.Current.KnownServers == nil {
		config.Current.KnownServers = make(map[string]config.ServerIdentity)
	}

	config.Current.KnownServers[certHash] = config.ServerIdentity{
		CertHash:    certHash,
		AuthCode:    authCode,
		LastIP:      serverIP,
...
func (pm *PinningManager) IsTrusted(certHash string) bool {
	...
	_, exists := config.Current.KnownServers[certHash]
	return exists
}
...
func (pm *PinningManager) RemoveTrust(certHash string) {
	...
	if config.Current.KnownServers != nil {
		delete(config.Current.KnownServers, certHash)
		config.Save()
	}
}
