...
### 🟣 The Core: Server
The heart of the system is the **Server** (`server`), a high-performance Go application that acts as the central hub.
...
The **Android App** (`android-app`) is the primary interface for sharing from your phone.
...
### 🔵 The Desktop Client: Desktop App
The **Desktop App** (`desktop-app`) is a modern GUI dashboard built with [Fyne](https://fyne.io).
...
*   **Interaction:**
    ...
    *   **Files:** Lists files from the server's shared directory.
...
*   **TOFU Certificate Pinning:** Both Android and Desktop clients implement **Trust On First Use** - on first connection, the server's certificate fingerprint is displayed for verification. Once trusted, subsequent connections verify the certificate hasn't changed, preventing man-in-the-middle attacks.
...
*   **Desktop App:**
    ...
    *   **Terminal UI (TUI):** A lightweight terminal interface for CLI workflows.
...
The server is controlled by `config.json` in the `server` directory.
...
*   **Frontend (Desktop App):** Go (Fyne GUI Toolkit & Bubbletea TUI)
...
5. **Get the Admin Code**: Open `config.json` and verify the `"admin_code"`.

### 💻 Desktop Client Setup

1. **Download**: Place `k-share-desktop.exe` (GUI) or `k-share-tui.exe` (TUI) anywhere.
2. **Run**: Launch the application.
3. **Connect**:
   - **Server IP**: Enter the IP address of the server PC.
   - **Pairing Code**: Enter the **Admin Code** or **Guest Code**.
4. **Trust Certificate**: Click **Trust** on the first connection prompt.
